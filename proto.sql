-- create the table
select min(micros) from logger where runid='rlp0115';
create table rlp0115 as select *, (micros-1201849275903587)/1000000.0 
       as secs from logger where runid='rlp0115';
alter table rlp0115 add column loopnum int(5), add column stage int(3);
create view runtbl as select * from rlp0115;

-- some indexes will help
create index nodeidx on rlp0115 (node);
create index sliceidx on rlp0115 (sliceid);
...
-- not needed?
update runtbl set 
       loopnum=convert(substr(custom from locate('||',custom)+2 
                              for locate('~~',custom)-(locate('||',custom)+2)),
                       signed) 
       where custom like '%loop%';

-- ??
update runtbl set loopnum=1 
       where node=1 and sliceid = 31 and secs > 10.4551 and 
             secs < 210.3329 order by secs;

select node, sliceid from runtbl where loopnum > 0 
       group by node, sliceid order by node, sliceid;
select secs,node,sliceid,substr(comment from 1 for 30) as msg,loopnum,stage 
       from runtbl where node=1 and sliceid=2;
--

-- combinations of node and sliceid:
select runid,node,sliceid from runtbl group by runid,node,sliceid order by sliceid,node;
...
begin
  ...
  declare rec cursor for select secs,custom,comment from rlp0128 
  ...
  repeat
    ...
    if cus like 'loop%' then
       update rlp0128 set loopnum=ln where node=nd and sliceid=sl and
    ...
    if comm like '%iStage %' then
       update rlp0128 set stage=stg where node=nd and sliceid=sl and
...
select t1.secs,t2.secs-t1.secs as duration,t1.date,t1.micros,t1.timereceived,
       t1.node,t1.sliceid,t1.loopnum,t1.stage from startloop t1, startloop t2 
  where t1.node=t2.node and t1.sliceid=t2.sliceid and t1.loopnum+1=t2.loopnum 
  order by t1.loopnum,t1.sliceid;
...
create view apploop as 
       select node,loopnum,sum(duration) as sum from durations 
       where name='app' group by node,loopnum order by node,loopnum;
insert into durations
       select d.runid,d.sliceid,d.date,d.micros,d.node,d.visitId,d.secs,
